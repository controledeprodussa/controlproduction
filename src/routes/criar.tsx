...
import { Plus, Trash2, Wrench, PackagePlus, Pencil } from "lucide-react";
...
function ModelosList() {
  ...
  const { data: models = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data: ms, error } = await supabase.from("machine_models").select("id, nome, created_at").order("created_at", { ascending: false });
      if (error) throw error;
